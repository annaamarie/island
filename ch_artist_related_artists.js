...
const dataFolder = path.join(process.cwd(), 'ch_artist_related_artists');
...
async function fetchAndSaveArtistData(artistId) {
  const url = `https://api.chartmetric.com/api/artist/${artistId}/relatedartists?limit=5&fromDaysAgo=7&toDaysAgo=0`;
  ...
  try {
    ...
    const fileName = `artist_${artistId}_related_artists.json`;
