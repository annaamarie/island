...
const dataFolder = path.join(process.cwd(), 'ch_artist_charts');
...
async function fetchAndSaveArtistData(artistId) {
  const url = `https://api.chartmetric.com/api/artist/${artistId}/spotify_viral_daily/charts?since=2024-01-01&until=2024-12-13`;
  ...
  try {
    ...
    const fileName = `artist_${artistId}_charts.json`;
