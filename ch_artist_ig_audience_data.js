...
/*
  ...
  - We use today's date and loop back 1 to 4 months.
  - We set the day to 15 as a consistent reference date (you can change this if needed).
  - We format the date as YYYY-MM-DD.
  - We create a label using a short month abbreviation (e.g., "jan") and the last two digits of the year.
  - We generate a folder name based on the label.
*/

const configs = []; // This will hold our dynamic configuration objects.
const today = new Date();
const monthNames = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"];

for (let i = 1; i <= 4; i++) {
  // Create a date for the i-th previous month, using the 15th day as a reference.
  let d = new Date(today.getFullYear(), today.getMonth() - i, 15);
  
  // Get year, month, and day components.
  let year = d.getFullYear();
  let month = d.getMonth() + 1; // JavaScript months are 0-indexed, so add 1.
  let day = d.getDate();

  // Format month and day with two digits (e.g., "09" for September).
  ...
  let folderName = `ch_artist_ig_audience_data_${label}`;
...
/*
  ...
  This function fetches the data for one artist using a specific configuration.
  - It constructs the API URL using the artist's ID and the date from the config.
  - It ensures the target folder exists (creates it if needed).
...
async function fetchAndSaveArtistData(artistId, config) {
  ...
  const url = `https://api.chartmetric.com/api/artist/${artistId}/instagram-audience-stats?date=${date}&geoOnly=false`;
  ...
  try {
    ...
    const fileName = `artist_${artistId}_ig_audience_data_${label}.json`;
...
/*
  FUNCTION: main
  This function iterates through each configuration and for each one, it fetches the data
